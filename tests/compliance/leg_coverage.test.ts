...
describe('leg coverage register', () => {
  ...
  test('the nested identity pairs in the real tree are exactly these, by name (BW-1 d)', () => {
    ...
    expect(nestedIdentities(legs), 'the nested identity pairs changed -- see the comment above').toEqual([
      ...
      'attest_counts.mjs "cannot read" ⊂ readback_admin.sh "this curl cannot read headers from a file (-H @file needs 7.55 or later), so the token would have to go on the command line -- nothing was sent"',
      ...
      'lint_audit_log_columns.sh "could not read" ⊂ readback_admin.sh "could not read packages/origins/publishable-keys.json (node exited"',
      ...
      'lint_from_allowlist.sh "could not read" ⊂ readback_admin.sh "could not read packages/origins/publishable-keys.json (node exited"',
      'lint_from_allowlist.sh "could not read" ⊂ readback_worker.sh "could not read the Supabase project ref from"',
      'readback_common.sh "the check did not run, so this read-back has no verdict" ⊂ readback_common.sh "listing the page\'s scripts -- the check did not run, so this read-back has no verdict"',
      ...
      'readback_common.sh "the check did not run, so this read-back has no verdict" ⊂ readback_common.sh "to compare against -- the check did not run, so this read-back has no verdict"',
