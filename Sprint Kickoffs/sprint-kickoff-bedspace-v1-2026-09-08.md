...
**Gate 3 — the 4am test.** With every seeded ward aged past 2 hours, a category filter returns results for all **ten** categories **[CORRECTED 2026-09-09 — ward-category audit]** (was "eight"; the kickoff never enumerated them, and an audit before the first hosted push split `ICU` into adult and paediatric and added `PAEDIATRIC`, which had no tile at all — see migration 002), badged grey, every row carrying a non-null E.164 number, in the exact expected order, with cardinality equal to the unfiltered nonzero count. Plus the property test: for any generated dataset, `count(search(cat)) >= count(rows where offered and bed_count > 0)`. Freshness must never reduce cardinality.
