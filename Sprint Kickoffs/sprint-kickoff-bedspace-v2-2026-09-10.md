...
Gate 2 requires *"the tile reflects it **within 60 seconds with no page refresh**"*. That was written against A1's Realtime design. **A1 was reversed**: public reads come from a static snapshot at `s-maxage=30, stale-while-revalidate=300`, which may legitimately serve a five-minute-old payload. **[CORRECTED 2026-09-21 (R-2026-09-21-43): the CONCLUSION stands and the REASON does not. `stale-while-revalidate` is DOCUMENTED as unsupported by `cache.put`/`cache.match`, and the zone answers `/beds.json` with `cf-cache-status: DYNAMIC`, so nothing at the edge serves a stale copy. The real worst case is the generator's 60s cadence + the cache's 30s TTL + the client's 30s poll, about **120 seconds** -- still more than 60, so the restatement holds, on half the bound and a different reason.]**
...
**The generator's death is camouflaged by correct UI behaviour, and this is the trap worth naming.** A dead generator degrades into grey badges, "last known", and at 24h "Status unknown — call to confirm" — exactly the states Bundle 4 is designed to render *calmly*. `stale-while-revalidate=300` keeps the CDN serving a plausible file for five minutes after the origin dies. **[CORRECTED 2026-09-21 (R-2026-09-21-43): FALSE, and the conclusion below is STRENGTHENED rather than weakened. SWR is unsupported by `cache.put`/`cache.match` and the zone does not cache this path, so the camouflage window is the cache's **30-second TTL**, after which the Function returns a `no-store` 5xx -- not five minutes of plausible file. The sensor argument is unchanged and now rests on less: a dead generator is invisible to the dashboard and to `generated_at` whatever the cache does, and `version` not incrementing is still the discriminating signal.]** So the sensor cannot be the dashboard, and it cannot be a timestamp either: a generator that runs and emits an identical payload looks fine on `generated_at`. **The discriminating signal is that `version` stopped incrementing**, backed by the transactional heartbeat. Both are in this stage rather than waiting for Bundle 5, because the slice is the first thing that can actually stop.
...
**The private disclosure address — moved on confirmed receipt, not on DNS.** `security@openbed.ng` was created on 2026-09-13. DNS alone was never treated as evidence. Early that day `openbed.ng` was a lame delegation refusing every query, so no mail to it could have been delivered. When it later answered — Proton Mail MX (`mail.protonmail.ch`, `mailsec.protonmail.ch`), nameservers at WhoGoHost, SOA serial `2026091310`, verified through two independent DNS-over-HTTPS resolvers with `toni.health` as a passing control — that showed only where mail is routed, not that it is received. **What moved the address was receipt: the founder sent a test message from an external address and confirmed on 2026-09-13 that it arrived.** `SECURITY.md` now names `security@openbed.ng`. **`toni.health` may lapse once that change is merged, and not before**: until then the default branch still carries the old address, and it is the one a researcher reads before reporting a vulnerability in this public repository.
...
**Not checkable from the repository (7):** the CDN cache headers; the `stale-while-revalidate` behaviour **[RECLASSIFIED 2026-09-21 (R-2026-09-21-43): this one IS checkable, and it is false — see the corrections at :136, :238 and :456]**; Supabase's pause rule; the unpapered DPA; "no production rows"; the free-tier connection limit; and the runtime OpenAPI observation. These are vendor or hosted facts; they are not repo state.
