...
4. **No later `ALTER TABLE app.referral ADD COLUMN`** in any forward migration. Straight lift from the audit-log guard, same explicit exit-code separation — 1 is a finding, anything else is fatal and loud, per the `lint_no_piped_grep_q.sh` ban.
