...
| 023 | `operator_register_location_and_phone` | **The operator register shows what is saved** (R-2026-09-24-98 BZ-2). `public.operator_register()` gains `lat`, `lng` and `public_phone_e164` on each facility, after `state`: 021's body verbatim with three keys added, so the admin app's edit form prefills them instead of asking the operator to retype the emergency number (BZ-1). All three are already public output (008:109-110), so no new personal data is exposed. Signature, return type and grants unchanged; `function-grants.json` unchanged, so fence 6 reads the same. The down restores 021's body verbatim. |
