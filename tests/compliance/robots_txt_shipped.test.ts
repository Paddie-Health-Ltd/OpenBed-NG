 ...
 */
const SOURCE = join(REPO_ROOT, 'apps', 'public-dashboard', 'public', 'robots.txt');
const BUILT = join(REPO_ROOT, 'apps', 'public-dashboard', 'dist', 'robots.txt');

describe('robots.txt', () => {
  test('the source file exists and disallows the whole site', () => {
    expect(existsSync(SOURCE), `no robots.txt source at ${SOURCE}`).toBe(true);
    const text = readFileSync(SOURCE, 'utf8');
    expect(text, 'robots.txt names no user-agent, so it applies to nobody').toMatch(/^User-agent:\s*\*$/m);
    expect(text, 'robots.txt allows the site it was written to disallow').toMatch(/^Disallow:\s*\/$/m);
  });

  test('the BUILT bundle carries it — the artefact a deploy uploads', () => {
    expect(
      existsSync(BUILT),
      `robots.txt is in the repository but not in dist — run \`npm run build\`; if the build ran, Vite's public/ copying is no longer reaching it, and the deployed site has no crawler directive at all`,
    ).toBe(true);
