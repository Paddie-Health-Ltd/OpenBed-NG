...
/**
 * GUARD OVER SECTION 8 of docs/runbook-cloudflare-pages-beds-json.md — deleting the
 ...
 *
 * THE SECOND IS THE ONE PEOPLE WOULD DROP, and it is the load-bearing half. Section 2
...
/** Section 8's text, from its heading to the next top-level heading. */
...
describe('runbook section 8 — deleting dead config', () => {
  test('anti-vacuity — section 8 is found, and it is not a stub', () => {
    ...
    expect(step, 'section 8 not found — this guard checked nothing').toBeDefined();
    expect((step ?? '').length, 'section 8 is too short to contain a gate and a re-observation').toBeGreaterThan(800);
  });

  test('it is GATED on section 5, and names what must be read back', () => {
    const step = stepEight(TEXT) ?? '';
    expect(step, 'the heading does not say it is gated').toMatch(/GATED by section 5/);
  ...
  test('it requires a REDEPLOY and a re-read AFTER the deletion — the half that makes it falsifiable', () => {
    ...
    // "You may re-run section 5" left the loose version of this assertion GREEN while
    ...
    expect(step, 'the step does not REQUIRE re-running section 5 afterwards').toMatch(/\*\*Re-run section 5's block\*\*/);
    expect(step, 'the re-read is offered rather than required').not.toMatch(/may re-run section 5/i);
  });
  ...
  test('its block carries NO `exit` — every fence here is pasted into an interactive shell', () => {
    ...
    // stopping a script. Section 8 is the only section that had one, and it does not now.
    const step = stepEight(TEXT) ?? '';
    const fences = [...step.matchAll(/```bash\n([\s\S]*?)```/g)].map((m) => m[1] ?? '');
    expect(fences.length, 'section 8 carries no bash fence, so there is nothing to paste').toBeGreaterThan(0);
    for (const fence of fences) {
      expect(fence, 'a fence in section 8 calls exit, which closes the shell it is pasted into').not.toMatch(/(^|\s)exit\s/);
    }
  });

  test('plant — a section 8 with its gate sentence removed is rejected', () => {
    const planted = TEXT.replace('GATED by section 5', 'a good idea after section 5');
    expect(planted, 'the plant did not mutate the document').not.toBe(TEXT);
    expect(stepEight(planted) ?? '', 'an ungated deletion step was accepted').not.toMatch(/GATED by section 5/);
  });

  test('plant — a section 8 with its post-deletion re-read removed is rejected', () => {
    const planted = TEXT.replace("**Re-run section 5's block**", '**Optionally re-run something**');
    expect(planted, 'the plant did not mutate the document — the anchor moved').not.toBe(TEXT);
    expect(stepEight(planted) ?? '', 'a deletion step with no re-observation was accepted').not.toMatch(/Re-run section 5/);
  });
});
